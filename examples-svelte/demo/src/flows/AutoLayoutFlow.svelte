<script lang="ts">
  ...
  const nodes = writable<Node[]>(elkNodes);
  const baseEdges = writable<Edge[]>(elkEdges);

  const styledEdges = writable<Edge[]>(applyEdgeType(elkEdges, edgeStyle));
  ...
  async function applyLayout(currentNodes: Node[]) {
    const laid = await runAutoLayout(currentNodes, $baseEdges, {
