 ...
 * Svelte Flow version — styles are CSS strings.
 */
...
function getNodeDims(node: Node): { width: number; height: number } {
  ...
  return {
    width: node.measured?.width ?? parseCssDimension(styleStr, "width") ?? 150,
    height: node.measured?.height ?? parseCssDimension(styleStr, "height") ?? 50,
  };
}

function setCssSize(style: string | undefined, width: number, height: number): string {
  let s = style ?? "";
  if (/width\s*:/.test(s)) {
    s = s.replace(/width\s*:\s*[^;]+;?/, `width: ${width}px;`);
  } else {
    s += ` width: ${width}px;`;
  }
  if (/height\s*:/.test(s)) {
    s = s.replace(/height\s*:\s*[^;]+;?/, `height: ${height}px;`);
  } else {
    s += ` height: ${height}px;`;
  }
  return s.trim();
