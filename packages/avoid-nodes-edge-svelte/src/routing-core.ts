...
export class PersistentRouter {
  ...
  reset(Avoid: AvoidLibInstance, nodes: FlowNode[], edges: FlowEdge[], options?: AvoidRouterOptions): Record<string, AvoidRoute> {
    this.Avoid = Avoid;
  ...
  destroy(): void {
    if (this.router) {
      cleanup(this.router as { deleteConnector: (c: unknown) => void; deleteShape: (s: unknown) => void; delete?: () => void }, this.connRefList, this.shapeRefList);
      this.router = null;
    }
  ...
  private buildRouter(): Record<string, AvoidRoute> {
    ...
    if (this.router) {
      try {
        for (const { connRef } of this.connRefList) (this.router as { deleteConnector: (c: unknown) => void }).deleteConnector(connRef);
        for (const { ref } of this.shapeRefList) (this.router as { deleteShape: (s: unknown) => void }).deleteShape(ref);
      } catch { /* ok */ }
    } else {
      this.router = new Avoid.Router(Avoid.OrthogonalRouting);
      const r = this.router as { setRoutingParameter: (p: number, v: number) => void; setRoutingOption: (o: number, v: boolean) => void };
    ...
    for (const edge of avoidEdges) {
      ...
      let sourcePos = getHandlePosition(src, "source");
      let targetPos = getHandlePosition(tgt, "target");
      if (autoBestSide) { const sb = getNodeBoundsAbsolute(src, this.nodeById); const tb = getNodeBoundsAbsolute(tgt, this.nodeById); const best = getBestSides(sb, tb); sourcePos = best.sourcePos; targetPos = best.targetPos; }
      let srcEnd: unknown, tgtEnd: unknown;
      if (splitNearHandle) {
        srcEnd = srcShapeRef ? new Avoid.ConnEnd(srcShapeRef, pinIdForPosition[sourcePos] ?? PIN_CENTER)
          : (() => { const sb = getNodeBoundsAbsolute(src, this.nodeById); const pt = getHandlePoint(sb, sourcePos); const p = new Avoid.Point(pt.x, pt.y); const end = new Avoid.ConnEnd(p); try { (p as { delete?: () => void }).delete?.(); } catch { /* ok */ } return end; })();
        tgtEnd = tgtShapeRef ? new Avoid.ConnEnd(tgtShapeRef, pinIdForPosition[targetPos] ?? PIN_CENTER)
          : (() => { const tb = getNodeBoundsAbsolute(tgt, this.nodeById); const pt = getHandlePoint(tb, targetPos); const p = new Avoid.Point(pt.x, pt.y); const end = new Avoid.ConnEnd(p); try { (p as { delete?: () => void }).delete?.(); } catch { /* ok */ } return end; })();
      } else {
        const sb = getNodeBoundsAbsolute(src, this.nodeById); const sourcePt = getHandlePoint(sb, sourcePos);
        const srcPt = new Avoid.Point(sourcePt.x, sourcePt.y); srcEnd = new Avoid.ConnEnd(srcPt); try { (srcPt as { delete?: () => void }).delete?.(); } catch { /* ok */ }
        const tb = getNodeBoundsAbsolute(tgt, this.nodeById); const targetPt = getHandlePoint(tb, targetPos);
        const tgtPt = new Avoid.Point(targetPt.x, targetPt.y); tgtEnd = new Avoid.ConnEnd(tgtPt); try { (tgtPt as { delete?: () => void }).delete?.(); } catch { /* ok */ }
      }
    ...
    try { (this.router as { processTransaction: () => void }).processTransaction(); } catch { this.destroy(); return {}; }
    return this.readRoutes();
  }
