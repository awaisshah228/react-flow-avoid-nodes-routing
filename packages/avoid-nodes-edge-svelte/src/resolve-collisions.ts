 ...
 * Svelte Flow compatible version using @xyflow/svelte types.
...
export function resolveCollisions(
  ...
  function resolveBoxes(boxes: Box[], checkRelated: boolean) {
    for (let iter = 0; iter <= maxIter; iter++) {
      ...
      for (let i = 0; i < boxes.length; i++) {
        for (let j = i + 1; j < boxes.length; j++) {
          const A = boxes[i];
          const B = boxes[j];
          if (checkRelated && areRelated(A.node, B.node, nodeById)) continue;
  ...
  for (const [, siblings] of siblingGroups) {
    ...
    resolveBoxes(buildBoxes(siblings, margin), false);
  }

  if (rootNodes.length >= 2) {
    resolveBoxes(buildBoxes(rootNodes, margin), true);
  }
